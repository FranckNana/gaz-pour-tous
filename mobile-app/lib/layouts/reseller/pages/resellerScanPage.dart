...
class ResellerScanPage extends StatefulWidget {
  ResellerScanPage({super.key, required this.targetStatus});

  BottleStatus targetStatus;

  @override
  State<ResellerScanPage> createState() => _ResellerScanPageState();
}

class _ResellerScanPageState extends State<ResellerScanPage> {
  ...
  _onQrRead(BarcodeCapture capture) async {
    ...
    if (mounted && code != null) {
      ...
      print("========== code  =========== $code");
      bool? verifyHash = await _bottlesController.validateBottleCode(code);
      if (verifyHash) {
        String? res = await _bottlesController.onResellerBottleScanned(
            widget.targetStatus, code, null);
        setState(() {
          isLoading = false;
        });
        String message;
